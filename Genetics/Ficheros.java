...
public class Ficheros {
    ...
    public static void guardarSolucionesMultiples(String nombre,ArrayList<Integer> s,Integer coste,Integer semilla,Integer numEvaluaciones,Integer iteracion) throws IOException {
      ...
      public static void guardarMediaMejor(ArrayList<Double> medias,ArrayList<Integer> mejor,String algoritmo) throws IOException {
        BufferedWriter bw=null;
        try {
            File fichero = new File("./zGraficas/"+ algoritmo + mejor.get(mejor.size()-1)+"  "+indice.size() + ".txt");
            
            bw = new BufferedWriter(new FileWriter(fichero));
            bw.write("Media;Mejor\n");
            for(int i=0;i<mejor.size();i++){
                bw.write(Math.round(medias.get(i))+";"+mejor.get(i)+"\n");
            }
            
            
        } catch (Exception ex) {
            System.out.println("error en la generacion del fichero media mejor ");
        } finally {
            if (null != bw) {
                bw.close();
            }
