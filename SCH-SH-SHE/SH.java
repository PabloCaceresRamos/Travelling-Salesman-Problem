...
public class SH {
    ...
    public ArrayList<Integer> calcularSH(int semilla, int numHormigas) throws InterruptedException, IOException {
        ...
        horaInicial.add(Calendar.MINUTE, 10);//Establecemos el tiempo de ejecución
        ...
        while(tiempoNoTerminado(horaInicial)){
            ...
            for (int i = 0; i < numHormigas; i++) {//Recorremos todas las hormigas
                ...
                for (int j = 1; j < caminoHormigaActual.size(); j++) {
                    ...
                    matrizFeromonas[j - 1][j] += aporte;
                    matrizFeromonas[j][j - 1] += aporte;
                }
    ...
    private boolean tiempoNoTerminado(Calendar horaInicial) {
       ...
       public void crearFicheroMejorTiempo() throws IOException{
        Ficheros.guardarMejorTiempo(mejor,"ch");
     }
